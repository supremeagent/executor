...
import { CircleStop, MessageCircleMore, Play, SendHorizontal, Sparkles } from 'lucide-react'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Input } from '@/components/ui/input'
import { Textarea } from '@/components/ui/textarea'
import { continueTask, createStream, executeTask, interruptTask, listEvents } from '@/lib/api'
import type { EventItem, ExecutorType, SessionItem } from '@/types/chat'

const SESSIONS_KEY = 'executor_frontend_sessions_v1'
...
export default function App() {
  ...
  function attachStream(sessionId: string) {
    streamRef.current?.close()
    const es = createStream(sessionId)
    ...
    es.onmessage = (evt) => {
      try {
        ...
        setMessages((prev) => {
          ...
          if (current.some((m) => m.seq === parsed.seq && parsed.seq !== undefined)) return prev
  ...
  async function handleFollowup() {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : '追加问题失败')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="page">
      ...
      <main className="layout">
        <Card className="sidebar">
          <CardHeader>
            ...
            <CardDescription>点击任意会话可查看完整消息和实时输出</CardDescription>
          </CardHeader>
              ...
              sessions.map((session) => (
                ...
                >
                  <div className="session-row">
                    ...
                    <Badge tone={session.status === 'running' ? 'running' : 'stopped'}>
                      {session.status}
                    </Badge>
                  </div>
                  <p>{session.id}</p>
        ...
        <section className="content">
          <Card>
            <CardHeader>
              ...
              <CardDescription>输入你的需求，创建一个新会话并立即开始流式监听</CardDescription>
            </CardHeader>
            <CardContent className="form-grid">
              ...
              <Button onClick={() => void handleCreateSession()} disabled={busy || !prompt.trim()}>
            ...
            <CardContent>
              <div className="actions">
                ...
                <Button
                  ...
                  disabled={!selectedSession || !followup.trim() || busy}
                >
                ...
                <Button
                  variant="outline"
                  onClick={() => selectedId && void handleSelectSession(selectedId)}
              ...
              <div className="message-panel">
                {loading ? (
                  ...
                  activeMessages.map((event, idx) => (
                    <article key={`${event.seq ?? 'n'}-${idx}`} className="message-item">
                      <header>
                        <Badge>{event.type}</Badge>
                        <small>{event.timestamp ? formatTime(event.timestamp) : 'streaming...'}</small>
                      </header>
      ...
      <footer className="footer">
        <span>React + TypeScript + shadcn-style components + Creative Tim theme flavor</span>
        <Button size="sm" variant="outline" onClick={() => void handleCreateSession()} disabled={busy || !prompt.trim()}>
