...
func TestBuildArgs_NoOptions(t *testing.T) {
	...
	if !containsFlag(args, "--experimental-acp") {
		t.Errorf("expected --experimental-acp flag, got: %v", args)
	}
	if containsFlag(args, "--yolo") {
		t.Errorf("expected no --yolo flag when Yolo=false, got: %v", args)
	}
}

func TestBuildArgs_WithYolo(t *testing.T) {
	args := buildArgs(executor.Options{Yolo: true})
	if !containsFlag(args, "--yolo") {
		t.Errorf("expected --yolo flag, got: %v", args)
	}
	if !containsFlag(args, "--allowed-tools") {
		t.Errorf("expected --allowed-tools flag, got: %v", args)
	}
	if !containsValue(args, "run_shell_command") {
		t.Errorf("expected 'run_shell_command' after --allowed-tools, got: %v", args)
	}
}

func TestBuildArgs_WithModel(t *testing.T) {
	args := buildArgs(executor.Options{Model: "gemini-2.0-flash"})
	if !containsFlag(args, "--model") {
		t.Errorf("expected --model flag, got: %v", args)
	}
	if !containsValue(args, "gemini-2.0-flash") {
		t.Errorf("expected model name in args, got: %v", args)
	}
}

func TestBuildArgs_ExtraArgs(t *testing.T) {
	args := buildArgs(executor.Options{ExtraArgs: []string{"--verbose"}})
...
func TestClient_Start_Integration(t *testing.T) {
	script := `printf '{"SessionStart":"sess-gemini"}\n' && printf '{"Message":{"Text":{"text":"hi from gemini"}}}\n'`
...
// TestClient_Start_WithYolo exercises the yolo/auto-approve path.
func TestClient_Start_WithYolo(t *testing.T) {
	script := `printf '{"SessionStart":"sess-1"}\n'`
	c := NewClient(fakeCmd(script))

	opts := executor.Options{WorkingDir: t.TempDir(), Yolo: true}
